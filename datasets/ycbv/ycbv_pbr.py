...
import normalSpeed
import json
from open3d_show_ycb import draw_correspond_dist_eror_gt
...
class YCBVDataset(torch.utils.data.Dataset):
    def __init__(self, data_cfg, dataset_name,demo_id=None):
        ...
        self.nn_dist_th = data_cfg["NN_DIST_TH"]
        ...
        for subset in (self.subsets):
           if subset=="train_real" :
                subset_pth = osp.join(data_cfg["DATA_ROOT_DIR"],subset)
                idx_file = osp.join(data_cfg["DATA_ROOT_DIR"],subset,'train.txt')
                self.real_annos += self.load_subset_dicts(idx_file,subset_pth)
           else :
                subset_pth = osp.join(data_cfg["DATA_ROOT_DIR"],subset)
                idx_file = osp.join(data_cfg["DATA_ROOT_DIR"],subset,'train.txt')
                self.pbr_annos += self.load_subset_dicts(idx_file,subset_pth)
        
        self.length = len(self.pbr_annos) + len(self.real_annos)
        print(f'Totally load {self.length} instences. real:{len(self.real_annos)} syn:{len(self.pbr_annos)}')
        if self.dataset_name == "test":
            self.test_annos = self.pbr_annos
       
        self.minibatch_per_epoch = self.length // data_cfg["BATCH_SIZE"]
        
        self.trancolor = transforms.ColorJitter(0.2, 0.2, 0.2, 0.05)
        self.obj_mesh = self.load_mesh(osp.join(data_cfg["DATA_ROOT_DIR"],'bop_ycb_kps'),self.selected_id)        
        self.rng = np.random
    ...
    def load_subset_dicts(self,idx_file,img_root):
        ...
        for scene_id, im_id in scene_im_ids:
            rgb_path = osp.join(img_root, f"{scene_id:06d}/rgb/{im_id:06d}.png")
            ...
            if "/train_real/" in rgb_path:
                img_type = "real"
            elif "/train_synt/" in rgb_path:
                img_type = "synt"
            elif "/train_pbr/" in rgb_path:
                img_type = "pbr"
            elif "/test/" in rgb_path:
                img_type = "test"
    ...
    def add_real_back(self, rgb, labels, dpt, dpt_msk):
        ...
        with Image.open(self.real_annos[real_item_idx]["depth_file"]) as di:
            real_dpt = np.array(di) / depth_factor
        ...
        with Image.open(self.real_annos[real_item_idx]["rgb_file"]) as ri:
            ...
            bk_rgb_clip = bk_rgb[rnd_h:(rnd_h+self.in_size),rnd_w:(rnd_w+self.in_size)]
            rgb_back = bk_rgb_clip * bk_clip[:, :, None]
        ...
        rgb = rgb * (msk_back == 0).astype(rgb.dtype) + rgb_back * msk_back
    ...
    def get_item(self, item_name, dtype=None):
        if dtype == None or dtype == 'pbr':
            anno = self.pbr_annos[item_name]
        else:
            anno = self.real_annos[item_name]
        ...
        # add noise
        if self.add_noise and img_typ == 'synt':
            rgb_clip = self.rgb_add_noise(rgb_clip)
            #print('add back')
            rgb_clip, dpt_m_clip = self.add_real_back(rgb_clip, mask_clip, dpt_m_clip, msk_dp_clip)
            if self.rng.rand() > 0.8:
                rgb_clip = self.rgb_add_noise(rgb_clip)

        rgb_clip = normalize_color(rgb_clip)
        dpt_m_clip = self.fill_missing(dpt_m_clip, 1, 1)
        msk_dp = dpt_m_clip > 1e-6

        ## calculate normals#
        dpt_mm_clip = (dpt_m_clip * 1000).astype(np.uint16)
        
        nrm_map = normalSpeed.depth_normal(
            dpt_mm_clip, K[0][0], K[1][1], 5, 2000, 20, False
        )
    ...
    def get_pose_gt_info(self, cld, pt_labels,anno):
        ...
        unfitted_cld_idx = np.where(dist > self.nn_dist_th)[0]
        #print(len(unfitted_cld_idx))
        ...
        mesh_match_idx[pt_labels==1] = np.array(matching_idx.copy(),dtype=np.int32)

        #filtered_pt_labels[filtered_pt_labels==255] = 1
        return filtered_pt_labels, mesh_match_idx, visible_flag,valid_data

    def __len__(self):
        return self.length

    def real_syn_gen(self, real_ratio=0.8):
       
        if self.rng.rand() < real_ratio:  # real
            n_imgs = len(self.real_annos)
            idx = self.rng.randint(0, n_imgs)
            return idx, 'real'
        else:
            idx = self.rng.randint(0, len(self.syn_annos))
            return idx, 'syn'

    def real_pbr_gen(self, real_ratio=0.8):
        ...
        if self.rng.rand() < real_ratio:  # real
            ...
            idx = self.rng.randint(0, len(self.pbr_annos))
            return idx, 'pbr'
    def __getitem__(self, idx):
        if self.dataset_name == 'train':
            id,dtype = self.real_pbr_gen()
            data = self.get_item(id,dtype)
            while data is None:
                id,dtype = self.real_pbr_gen()
                data = self.get_item(id,dtype)
            return data
