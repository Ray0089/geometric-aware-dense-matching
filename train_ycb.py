...
import ref
import shutil
from datasets.ycbv.ycbv_pbr import YCBVDataset as dataset_desc
...
parser.add_argument(
    "-dataset_name", type=str, default='ycbv',
    help="train or evaluate the model"
)
parser.add_argument(
    ...
    if os.path.isfile(filename):
        ...
        if model is not None and checkpoint["model_state"] is not None:
            ...
            if 'module' in list(ck_st.keys())[0]:
                ...
                ck_st = tmp_ck_st
            model.load_state_dict(ck_st,strict=False)
        ...
        with tqdm.tqdm(range(start_epoch,n_epochs), desc="epochs") as tbar:
            for epoch in tbar:
                ...
                for it,batch in enumerate(train_loader):
                    ...
                    #sum_match_loss += res['loss_matches']
                    # if res['loss_matches'] != 0:
                    #     item_num +=1
                    #seg_loss = out_dict['seg_loss'].item()
                    #match_loss = out_dict['match_loss'].item()
                    if (it+1) % 100 == 0:
...
def test(ref_data):
    ...
    cu_dt = {}
    #obj2id = ref_data.obj2id
    for id in cfg.DATASETS['OBJS'].keys():
        #cls_id = obj2id[cfg.DATASETS['OBJS'][i]]
